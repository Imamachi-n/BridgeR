...
BridgeRCalibrationWithPvalue <- function(InputFile="BridgeR_6_HalfLife_Pvalue_estimation.txt",
                                         group,
                                         hour,
                                         ComparisonFile,
                                         InforColumn=4,
                                         CutoffDataPoint=4,
                                         OutputFile="BridgeR_7_HalfLife_Calibration.txt"){
    ###Prepare_file_infor###
    ...
    for(SampleNum in comp_file_number){
        ...
        infor_st <- 1 + (SampleNum - 1)*(TimeDataSize + InforColumn + 13)
        infor_ed <- (InforColumn)*SampleNum + (SampleNum - 1)*(TimeDataSize + 13)
        ...
        halflife_index <- infor_ed + TimeDataSize + 8
            ...
            if(data_point >= CutoffDataPoint){
                if(as.numeric(as.vector(as.matrix(DataFrame$exp[1]))) > 0){
                    ...
                    if(flg == 0){
                        ...
                        fixed_halflife <- half_life
                        halflife_value <- log(0.5)
                    }else if(flg == 1){
                        half_life_w <- half_life/correction_value
                        fixed_halflife <- half_life/correction_value
                        halflife_value <- -coef*fixed_halflife
                    }

                    if(coef < 0){
                        fixed_halflife <- Inf
                        half_life_w <- 24
                    }else if(fixed_halflife > 24){
                        half_life_w <- 24
                    }
                    cat(model,coef,coef_error,coef_p,
                        r_squared,adj_r_squared,residual_standard_err,
                        half_life_w,fixed_halflife,
                        sep="\t", file=output_file, append=T)
                    ...
                    halflife_SD_minus <- fixed_halflife - predicted_exp_lwr_halflife #
                    halflife_SD_plus <- predicted_exp_upr_halflife - fixed_halflife #
                    ...
                    return(c(half_life_w, fixed_halflife, predicted_exp_df, halflife_SD_minus, halflife_SD_plus))
                }
            }
        }
    }
    ...
    for(index in 1:gene_number){
        ...
        for(SampleNum in comp_file_number){
            ...
            if(is.na(model)){
                ...
                ttest_infor <- pvalue_calc(TimePoint_Exp_data_base, flg)
                ...
                ttest_infor <- pvalue_calc(TimePoint_Exp_data_del, flg)
...
###fixed_halflife <- fitted_model_halflife/0.9
###fixed_halfexp <- exp(-fitted_model_coef*fixed_halflife)
